...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const navItems = [
    { path: "/", label: "儀表板", icon: <LayoutDashboard className="h-5 w-5 mr-3" /> },
    { path: "/appointments", label: "預約管理", icon: <Calendar className="h-5 w-5 mr-3" /> },
    { path: "/patients", label: "患者管理", icon: <Users className="h-5 w-5 mr-3" /> },
    { path: "/queue", label: "看診進度", icon: <MessageSquare className="h-5 w-5 mr-3" /> },
    { path: "/registration", label: "掛號系統", icon: <Info className="h-5 w-5 mr-3" /> },
    { path: "/reports", label: "報表分析", icon: <BarChart3 className="h-5 w-5 mr-3" /> },
    { path: "/settings", label: "系統設定", icon: <Settings className="h-5 w-5 mr-3" /> },
  ];
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center p-4 border-b border-neutral-100">
          ...
          <div>
            <h1 className="text-lg font-semibold text-neutral-800">康健聯合診所</h1>
        ...
        <div className="p-4 border-t border-neutral-100">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-neutral-700">{user?.fullName || "使用者"}</p>
              <p className="text-xs text-neutral-500">{user?.department || "員工"}</p>
            </div>
            <button 
